...
let tempDir = '';
beforeAll(async () => {
  tempDir = await setupTempDir(cmd);
});
afterAll(async () => void (await tearDownTempDir(tempDir)));
...
describe(`${cmd} integration tests:`, () => {
  ...
  describe('success cases', () => {
    test.only('should return help when called with the help flag', async () => {
      const { stdout } = await mockShell({
        cmd: `${cli} ${cmd} --help`,
        dir: tempDir,
      });

      expect(stdout).toContain('Usage: rapidstack create');
      expect(stdout).toContain('--template-loc <path, repository>');
      expect(stdout).toContain('--template <template-name>');
    });
